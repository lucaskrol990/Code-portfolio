random_removal_L <- function(sol) { 
  q_possible_up <- max(round(0.5 * n), 6) # large version, remove at most 50%
  q_possible_down <- max(5, round(0.3 * n)) # dont want overlap with medium
