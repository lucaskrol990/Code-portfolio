# This function removes the nodes that are most early at their location
early_remove <- function(sol, q = 1, n = 3) {
  three_most_early <- sort(sol[[11]], decreasing = T)[1:n]
  remove_node <- which(sol[[11]] == sample(three_most_early, q))
