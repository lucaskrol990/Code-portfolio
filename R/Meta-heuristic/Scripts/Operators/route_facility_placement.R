# For a complete route, fix charger locations. Do this by dynamic programming
route_facility_placement <- function(sol, tabu) {
  if (is.list(sol[[1]]) == F) {
    sol[[1]] <- list(sol[[1]])
  }
  ...
  for (i in length(route_changed):2) { # Start at the end
    ...
    while (j <= length(charge_vec)) { # Loop through the charge_vec
      ...
      if (charge_vec[j] > B + 0.001) { # This option is infeasible
